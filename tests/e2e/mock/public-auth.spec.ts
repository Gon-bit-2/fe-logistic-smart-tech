...
test("loads the landing page and uses operations navigation", async ({ page }) => {
  ...
  await page.goto("/vi/tracking");
  await expect(page.getByRole("link", { name: "Precision Logistics" })).toHaveAttribute(
    "href",
    "/vi",
  );
});
...
test("logs in and redirects to order creation", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/vi\/overview$/, { timeout: 20_000 });
...
test("completes register and forgot-password OTP flows", async ({ page }) => {
  ...
  await page.goto("/vi/auth/forgot-password");
...
test("tracks public shipments for success and not-found states", async ({ page }) => {
  ...
  await expect(page.getByText("Đang trung chuyển").first()).toBeVisible();
